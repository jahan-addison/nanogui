...
MessageDialog::MessageDialog(Widget* parent,
    Type type,
    ...
    : Window(parent, title)
{
    ...
    if (alt_button) {
        Button* button = new Button(
            panel2, alt_button_text, m_theme->m_message_alt_button_icon);
        button->set_callback([&] {
            if (m_callback)
                m_callback(1);
            dispose();
    ...
    Button* button =
        new Button(panel2, button_text, m_theme->m_message_primary_button_icon);
    button->set_callback([&] {
        if (m_callback)
            m_callback(0);
        dispose();
